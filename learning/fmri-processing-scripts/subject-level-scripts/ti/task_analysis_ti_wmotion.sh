...
while [[ $# -gt 0 ]]; do
	...
	sessioncount=1 #0 if I want to run both sessions, 1 if I want to run post only
	...
	while [[ $sessioncount -lt $numsess ]]; do #scrolls through each scanning session
		...
		while [[ $taskcount -lt $numtask ]]; do #scrolls through each scanning run
...
#~~~ Start if statement that skips first-level analyses for sessions that have already been run
			...
			if [ ! -d $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-"$smooth"mm.feat ]; then
		            	...
		            	python $outputdir/physics-learning/make-REAS.py $outputdir/data/behavioral-data/rcsv/$1/session-$sessioncount/${task^^}/${task^^}'_'$((taskcount+1)).csv $outputdir/data/behavioral-data/vectors/$1/session-$sessioncount/$task/$task-$taskcount
		            	if [[ "$1" -eq "339" ]]; then
		            		python $outputdir/physics-learning/make-REAS.py $outputdir/data/behavioral-data/rcsv/$1/session-$((sessioncount-1))/${task^^}/${task^^}'_'$((taskcount+1)).csv $outputdir/data/behavioral-data/vectors/$1/session-$sessioncount/$task/$task-$taskcount
		            	fi

		            	cp -a template_task_reas_wmotion.fsf $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-"$smooth"mm.fsf
				...
				if [ -e $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$1-session-$sessioncount-motion-outliers-censored.txt ]; then
			        ...
			        sed -i -e '313s/.*/set fmri(custom1) "\/home\/data\/nbc\/physics-learning\/data\/behavioral-data\/vectors\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'-Reasoning.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-"$smooth"mm.fsf
			        sed -i -e '362s/.*/set fmri(custom2) "\/home\/data\/nbc\/physics-learning\/data\/behavioral-data\/vectors\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'-Baseline.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-"$smooth"mm.fsf
...
#~~~ Start if statement that skips second-level analysis for sessions that have already been run
	        ...
	        cp -a template_task_sub_reas.fsf $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-"$smooth"mm.fsf
	        ...
	        sed -i -e '282s/.*/set feat_files(1) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-0\/'$task'-'$smooth'mm.feat"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-"$smooth"mm.fsf
		sed -i -e '285s/.*/set feat_files(2) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-1\/'$task'-'$smooth'mm.feat"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-"$smooth"mm.fsf
