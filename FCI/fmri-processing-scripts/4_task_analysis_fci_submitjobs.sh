# Submits job for fci task analysis
# Useage: bash 4_task_analysis_fci_submitjobs.sh -sub ### ### ### ...
...
while [[ $# -gt 0 ]]; do
	...
	if [[ "$1" != "-sub" ]]; then
		...
		sed -i -e '31s/.*/bash 4_task_analysis_fci\.sh '$1'/' $outputdir/data/first-level/$1/submitjob_task_analysis_$task.sub 
