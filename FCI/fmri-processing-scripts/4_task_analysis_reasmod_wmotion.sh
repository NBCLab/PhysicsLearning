...
while [[ $# -gt 0 ]]; do
	...
	while [[ $sessioncount -lt $numsess ]]; do #scrolls through each scanning session
		...
		### start reas  modulator
		## 1st level analysis
 			if [ ! -d $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-reasmod-s123-"$smooth"mm.feat ]; then
				...
				python $outputdir/physics-learning/make-FCImodulator_allscreens.py $outputdir/data/behavioral-data/rcsv/$1/session-$sessioncount/${task^^}/${task^^}'_'$((taskcount+1)).csv $outputdir/data/behavioral-data/vectors/$1/session-$sessioncount/$task/$task-$taskcount $outputdir/data/behavioral-data/fci_survey/FCI_Survey_Entries_Scaled_TabDelim.txt $1 $sessioncount
 				...
 				cp -a template_task_wreasmod_wmotion.fsf $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 				...
 				sed -i -e '33s/.*/set fmri(outputdir) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'\/'$task'-reasmod-s123-'$smooth'mm"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 				sed -i -e '109s/.*/set fmri(smooth) '$smooth'/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        sed -i -e '270s/.*/set feat_files(1) "\/home\/data\/nbc\/physics-learning\/data\/pre-processed\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'\/'$task'"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        if [ -e $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$1-session-$sessioncount-motion-outliers-censored.txt ]; then
 			        	sed -i -e '276s/.*/set confoundev_files(1) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'\/'$1'-session-'$sessioncount'-motion-outliers-censored.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        fi
 			        sed -i -e '279s/.*/set highres_files(1) "\/home\/data\/nbc\/physics-learning\/data\/pre-processed\/'$1'\/session-'$sessioncount'\/anatomical\/'$anatdir'\/fsl\/anatomical-or_brain"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        sed -i -e '313s/.*/set fmri(custom1) "\/home\/data\/nbc\/physics-learning\/data\/behavioral-data\/vectors\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'-Physics.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        sed -i -e '365s/.*/set fmri(custom2) "\/home\/data\/nbc\/physics-learning\/data\/behavioral-data\/vectors\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'-NonPhysics.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 			        sed -i -e '417s/.*/set fmri(custom3) "\/home\/data\/nbc\/physics-learning\/data\/behavioral-data\/vectors\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-'$taskcount'-physStrategyReas-allscreens.txt"/' $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf

 			        feat $outputdir/data/first-level/$1/session-$sessioncount/$task/$task-$taskcount/$task-$taskcount-reasmod-s123-"$smooth"mm.fsf
 ...
 ## 2nd level analysis
 		if [ -d $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.gfeat ]; then
 			rm -rf $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.gfeat

 		fi

 	        cp -a template_task_sub_modulator.fsf $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf

 	        sed -i -e '33s/.*/set fmri(outputdir) "\/home\/data\/nbc\/physics-learning\/data\/second-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$1'-session-'$sessioncount'-'$task'-reasmod-s123-'$smooth'mm"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf
 	        sed -i -e '281s/.*/set feat_files(1) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-0\/'$task'-reasmod-s123-'$smooth'mm.feat"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf
 		sed -i -e '284s/.*/set feat_files(2) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-1\/'$task'-reasmod-s123-'$smooth'mm.feat"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf
 		sed -i -e '287s/.*/set feat_files(3) "\/home\/data\/nbc\/physics-learning\/data\/first-level\/'$1'\/session-'$sessioncount'\/'$task'\/'$task'-2\/'$task'-reasmod-s123-'$smooth'mm.feat"/' $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf

 	        feat $outputdir/data/second-level/$1/session-$sessioncount/$task/$1-session-$sessioncount-$task-reasmod-s123-"$smooth"mm.fsf
		## end reas modulator analysis
