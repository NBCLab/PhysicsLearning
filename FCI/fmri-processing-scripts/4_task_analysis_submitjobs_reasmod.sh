...
while [[ $# -gt 0 ]]; do
	...
	if [[ "$1" != "-sub" ]]; then
		...
		sed -i -e '6s/.*/#BSUB \-J '$1'-rm-'$task'/' $outputdir/data/first-level/$1/submitjob_task_analysis_$task.sub 
		sed -i -e '9s/.*/#BSUB \-eo \/home\/data\/nbc\/physics-learning\/data\/first\-level\/errorfiles\/eo\-'$1'-reasmodulator-123-'$task'-analysis/' $outputdir/data/first-level/$1/submitjob_task_analysis_$task.sub 
		sed -i -e '12s/.*/#BSUB \-oo \/home\/data\/nbc\/physics-learning\/data\/first\-level\/outputfiles\/oo\-'$1'-reasmodulator-123-'$task'-analysis/' $outputdir/data/first-level/$1/submitjob_task_analysis_$task.sub 
		sed -i -e '31s/.*/bash task_analysis_reasmod_wmotion\.sh '$1'/' $outputdir/data/first-level/$1/submitjob_task_analysis_$task.sub 
